...
function configureRepoTools(server: McpServer, tokenProvider: () => Promise<string>, connectionProvider: () => Promise<WebApi>, userAgentProvider: () => string) {
  ...
  server.tool(
    ...
    async ({ searchText, project, repository, branch, author, commitStartDate, commitEndDate, orderBy, includeFacets, skip, top }) => {
      ...
      const connection = await connectionProvider();
      // almsearch.dev.azure.com is hosted-only; on-prem collections have no equivalent endpoint.
      const url = buildHostedSearchUrl(connection.serverUrl, "commitSearchResults", apiVersion);
      if (!url) {
        return {
          content: [{ type: "text", text: "Commit search is currently supported only for Azure DevOps Services organizations." }],
          isError: true,
        };
      }
